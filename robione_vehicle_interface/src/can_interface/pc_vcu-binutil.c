...
{
 uint32_t recid = 0;
 if ((_id >= 0x10001U) && (_id < 0xC150011U)) {
  if ((_id >= 0x10001U) && (_id < 0x80A000FU)) {
   if (_id == 0x10001U) {
    recid = Unpack_SAFETY_MANAGER_STATUS_pc_vcu(&(_m->SAFETY_MANAGER_STATUS), _d, dlc_);
   } else if (_id == 0x40A0001U) {
